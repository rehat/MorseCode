...
-(void)sendMessage{
    ...
    if(![self.message isValid]){
        ...
        [self toggleTorch];
        self.message = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(next:) userInfo:codedMessage repeats:NO];
    }
}
...
-(void)next:(NSTimer *)timer{
    ...
    if ([timer.userInfo count] != 0) {
        ...
        if ([[timer.userInfo objectAtIndex:0] isEqualToString: @"dot"]) {
            [self toggleTorch];
            self.message=[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(next:) userInfo:timer.userInfo repeats:NO];
        }
        else if ([[timer.userInfo objectAtIndex:0] isEqualToString: @"dash"]) {
            [self toggleTorch];
            self.message=[NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(next:) userInfo:timer.userInfo repeats:NO];
        }
            ...
            self.message=[NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(next:) userInfo:timer.userInfo repeats:NO];
        }
